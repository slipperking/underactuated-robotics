    ...
    */
    ...
    if target() == "html" {
      let items = for i in range(len) {
        ...
        // Build data-bullet string from the numbering pattern
        let bullet-str = {
          let n = std.numbering(it.numbering, child-number)
          if n.has("text") { n.text } else if n.has("children") {
            n.children.map(c => if c.has("text") { c.text } else { "" }).join()
          } else { str(child-number) }
        }

        let li-body = next-show(child.body)
        ...
        (html.elem("li", attrs: (data-bullet: bullet-str), li-body),)
      }
      html.elem("ul", ..items, attrs)
    } else {
      // each item
